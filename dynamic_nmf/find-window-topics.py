...
def main():
    ...
    if len(kparts) == 1:
        ...
        if not options.model_path is None:
            ...
            validation_measure = unsupervised.coherence.WithinTopicMeasure(
                unsupervised.coherence.ModelSimilarity(model))
    ...
    for matrix_filepath in args:
        ...
        for k in range(actual_kmin, actual_kmax + 1):
            ...
            unsupervised.nmf.save_nmf_results(results_out_path, doc_ids, terms, term_rankings, partition, impl.W,
                                              impl.H, topic_labels)
        ...
        if len(coherence_scores) > 0:
            ...
            log.info(
                "- Top recommendations for number of topics for '%s': %s" % (window_name, ",".join(map(str, top_k))))
            selected_ks.append([matrix_filepath, top_k[0]])

    if not options.path_selected_ks is None:
        log.info("Writing selected numbers of topics for %d window datasets to %s" % (
        len(selected_ks), options.path_selected_ks))
